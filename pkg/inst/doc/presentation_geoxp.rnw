...
\title{Some examples of use GeoXp}
...
library(GeoXp)
@
Basically, a function\footnote{We are talking about interactive functions of \pkg{GeoXp}.
\pkg{GeoXp} contains also some internal or wrap functions which user should not be interested by.}
of the \pkg{GeoXp} package could be called by using this code:
\begin{verbatim}
function(sp.obj,...,
names.attr=names(sp.obj), criteria=NULL, carte=NULL, identify=FALSE, cex.lab=0.8,
pch=16, col="lightblue3", xlab="angle", ylab="absolut magnitude",
...
of spatial units. For drawing a map, the spatial coordinates of spatial units have been
extracted from \code{sp.obj}, by using the function \code{coordinates}. The characteristics
of spatial units are included in \code{sp.obj@data}, a \code{data.frame}. \newline

The \code{...} correspond to specificities of each function (more details in ~\ref{sec2}).
For example, some functions make analysis on one variable only. In this case, the \code{...}
will indicate first the name of the variable of interest and also some specificities of the
graphic used, like the number of bars for histogram, the representation or no of a quantile regression
curve for cloud points, etc...
\newline

Now, let see the options.

\subsection{Options}
 \begin{itemize}
  \item{names.attr}{a vector of character of size the number of variables included in \code{sp.obj@data}:
  for changing the names of variables included in \code{sp.obj@data}}
  ...
  \item{carte}{In the case where \code{sp.obj} is a \code{SpatialPolygonDataFrame}, user will have the
  ...
  \item{identify}{if TRUE, the names of selected sites will be printed on the map. The names of spatial units
  correpond to \code{row.names} of the attribute table \code{row.names(sp.obj@data)}.}
  \item{cex.lab}{a numeric value: it gives the character size of labels}
  \item{pch}{16 by default, it gives the symbol for selected points}
  \item{col}{"lightblue3" by default, it gives the color of a graphic. In the case where the variable of interest
  is a factor user could give a vector of colors corresponding to the colors of each level to be printed
  on the map.}
  \item{xlab}{a character: title for the graphic x-axis}
  \item{ylab}{a character: title for the graphic y-axis}
  \item{axes}{a boolean with TRUE for drawing axes on the map}
  \item{lablong}{a character: name of the x-axis that will be printed on the map}
  \item{lablat}{a character: name of the y-axis that will be printed on the map}
\end{itemize}

\subsection{A basic example}
This first example has been taken from the example help of \code{histomap} function.
We consider a data set containing price indices of real estate from biggest cities in France in 2008.
...
@
\end{footnotesize}
This data set is a \code{data.frame} containing the spatial coordinates of the
cities in variables \code{longitude} and \code{latitude}. It contains also several
variables corresponding the name of cities, a number of region, average price of sell and
rent, etc...
...
A call to a \pkg{GeoXp} function generally opens three windows: two graphical \RR~ windows for the
...
\section{Descriptive functions}\label{sec2}
...
W <- makeneighborsw(cbind(x.OLD , y.OLD),method="neighbor",4)
